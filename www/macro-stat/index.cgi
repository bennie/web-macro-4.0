...
my $tmpl   = new Macro::Template({
  template_file => "$FindBin::Bin/../templates/main-template.tmpl",
  html_prefix   => '/',
  image_prefix  => '/images/'
});
...
my $body = $cgi->start_form
         . $cgi->div(
             { -class => 'macrostat-card' },
             ( $title ? $cgi->div({ -class => 'macrostat-card-head' }, $title) : '' ),
             $cgi->div({ -class => 'macrostat-card-body' }, @body)
           )
         . $cgi->end_form;
...
sub basic {
  $title = title_h2('The Basic Stats');
  return 
...
sub modify {
  $title = title_h2('The Changing Stat');
  return 
...
sub title_h2 {
  ...
  return $cgi->div(
    ...
    $cgi->h2({ -style => 'color:#FFFFFF; margin:0;' }, $text)
  );
}
